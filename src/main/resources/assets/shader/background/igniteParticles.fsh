...
vec4 blend(vec4 src, vec4 dst) {
    float srcAlpha = src.z * backgroundAlpha;
...
void main()
{
    ...
    vec4 backgroundColor = texelFetch(background, texturePosition, 0);

    fragColor = vec4(col, (col.x * 0.3 + col.y * 0.6 + col.z * 0.1) * backgroundAlpha);
}