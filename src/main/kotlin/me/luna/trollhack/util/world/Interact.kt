...
import me.luna.trollhack.util.collections.EnumSet
...
fun SafeClientEvent.getNeighborSequence(
    ...
    getNeighborSequence(player.eyePosition, pos, attempts, range, visibleSideCheck, entityCheck, sides, ArrayList(), pos, 0)
...
private fun SafeClientEvent.getNeighborSequence(
    ...
    if (attempts > 1) {
        for (side in sides) {
            ...
            val placeInfo = checkNeighbor(eyePos, pos, side, range, visibleSideCheck, entityCheck, false, origin, lastDist)
                ?: continue
            val newSequence = ArrayList(sequence)
            newSequence.add(placeInfo)

            return getNeighborSequence(eyePos, newPos, attempts - 1, range, visibleSideCheck, entityCheck, sides, newSequence, origin, lastDist + 1)
                ?: continue
        }
    }
...
private fun SafeClientEvent.getNeighbor(
    ...
    if (attempts > 1) {
        for (side in sides) {
            ...
            return getNeighbor(eyePos, newPos, attempts - 1, range, visibleSideCheck, entityCheck, sides, origin, lastDist + 1)
                ?: continue
        }
    }
