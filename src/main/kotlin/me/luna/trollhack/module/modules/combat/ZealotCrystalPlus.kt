...
import me.luna.trollhack.util.combat.CombatUtils.totalHealth
import me.luna.trollhack.util.combat.CrystalDamage
import me.luna.trollhack.util.combat.CrystalUtils
import me.luna.trollhack.util.combat.CrystalUtils.canPlaceCrystalOn
import me.luna.trollhack.util.combat.CrystalUtils.isResistant
...
import me.luna.trollhack.util.items.attackDamage
import me.luna.trollhack.util.items.duraPercentage
...
import me.luna.trollhack.util.world.rayTraceVisible
            ...
            if (checkBreakRange(packet.x, packet.y, packet.z, mutableBlockPos)) {
                if (!paused() && !bbtt && checkCrystalRotation(packet.x, packet.y, packet.z)) {
                    placeInfo.getLazy()?.let {
                        when (packetBreak) {
                            BreakMode.TARGET -> {
                                if (CrystalUtils.crystalPlaceBoxIntersectsCrystalBox(it.blockPos, packet.x, packet.y, packet.z)) {
        ...
        runSafe {
            ...
            if (placeInfo != null) {
                if (distanceSq(
                    ...
                    if (attackedPosMap.containsKey(toLong(packet.x.fastFloor(), packet.y.fastFloor(), packet.z.fastFloor()))) {
                        explosionCount++
                    }
    ...
    private fun SafeClientEvent.getCrystal(crystalList: List<EntityEnderCrystal>): EntityEnderCrystal? {
        ...
        if (targets.isNotEmpty()) {
            for (crystal in crystalList) {
                val selfDamage = max(context.calcDamage(crystal.posX, crystal.posY, crystal.posZ, false, mutableBlockPos), context.calcDamage(crystal.posX, crystal.posY, crystal.posZ, true, mutableBlockPos))
    ...
    private fun SafeClientEvent.getTargets(): Sequence<TargetInfo> {
        ...
        if (mobs || animals) {
            for (target in EntityManager.entity) {
                ...
                list.add(TargetInfo(target, pos, target.entityBoundingBox, pos, Vec3d.ZERO, ExposureSample.getExposureSample(target.width, target.height)))
            }
        }
