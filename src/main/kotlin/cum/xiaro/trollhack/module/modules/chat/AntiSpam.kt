...
import cum.xiaro.trollhack.util.text.SpamFilters
        ...
        safeListener<PacketEvent.Receive> { event ->
            ...
            if (pattern != null) { // null means no pattern found
                if (mode == Mode.HIDE) {
                    ...
                    event.packet.textComponent = TextComponentString(sanitize(event.packet.textComponent.formattedText, pattern, replaceMode.redaction))
                }
            }
            ...
            if (fancyChat) {
                ...
                if (message.trim { it <= ' ' }.isEmpty()) { // this should be removed if we are going for an intelligent de-fancy
                    event.packet.textComponent = TextComponentString(getUsername(event.packet.textComponent.unformattedText) + " [Fancychat]")
                }
            }
        }
    }
