...
import dev.luna5ama.trollhack.module.modules.combat.Burrow
...
internal object Speed : Module(
    ...
    private val stepTimeout by setting("Step Timeout", 10, 0..100, 1, page.atValue(Page.STRAFE))
    ...
    private val minBoostSpeed by setting(
        ...
        0.0..1.0,
        0.01,
        page.atValue(Page.BOOST) and { velocityBoost > 0.0 })
    private val maxBoostSpeed by setting(
        "Max Boost Speed",
        0.6,
        0.0..1.0,
        0.01,
        page.atValue(Page.BOOST) and { velocityBoost > 0.0 })
    private val maxYSpeed by setting(
        "Max Y Speed",
        0.5,
        0.0..1.0,
        0.01,
        page.atValue(Page.BOOST) and { velocityBoost > 0.0 })
    private val boostDelay by setting(
        "Boost Delay",
        250,
        0..5000,
        50,
        page.atValue(Page.BOOST) and { velocityBoost > 0.0 })
    private val boostTimeout by setting(
        "Boost Timeout",
        500,
        0..2500,
        50,
        page.atValue(Page.BOOST) and { velocityBoost > 0.0 })
    private val boostDecay by setting(
        "Boost Decay",
        0.98,
        0.1..1.0,
        0.001,
        page.atValue(Page.BOOST) and { velocityBoost > 0.0 })
    private val boostRange by setting(
        "Boost Range",
        4.0f,
        1.0f..8.0f,
        0.1f,
    ...
    init {
        ...
        safeListener<PacketEvent.Receive> {
            ...
            when (it.packet) {
                ...
                is SPacketExplosion -> {
                    handleVelocity(
                        it,
                        Vec3d(it.packet.x, it.packet.y, it.packet.z),
                        hypot(it.packet.motionX.toDouble(), it.packet.motionZ.toDouble()),
                        it.packet.motionY.toDouble()
                    )
                }
                is SPacketEntityVelocity -> {
                    if (it.packet.entityID == player.entityId) {
                        handleVelocity(
                            it,
                            null,
                            hypot(it.packet.motionX / 8000.0, it.packet.motionZ / 8000.0) - moveSpeed,
                            it.packet.motionY / 8000.0
                        )
                    }
                }
            }
        ...
        safeListener<PlayerMoveEvent.Pre> {
            ...
            if (shouldStrafe()) {
                ...
                updateState(baseSpeed, dirX, dirZ)
                ...
                it.x = dirX * boostedSpeed
                it.z = dirZ * boostedSpeed
    ...
    private fun SafeClientEvent.handleVelocity(
        event: PacketEvent.Receive,
        pos: Vec3d?,
        speed: Double,
        motionY: Double
    ) {
    ...
    private fun SafeClientEvent.updateFinalSpeed(baseSpeed: Double) {
        if (!isBurrowed()) {
            ...
            if (prevCollided
                && stepTicks < stepTimeout
                && stepPauseTicks > stepPause
                && reverseStepPauseTicks > reverseStepPause
                && rubberBandTicks > 5
            ) {
