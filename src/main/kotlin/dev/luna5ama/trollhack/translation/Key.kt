...
import dev.luna5ama.trollhack.translation.TranslationManager.getTranslated
import dev.luna5ama.trollhack.util.IDRegistry
import dev.luna5ama.trollhack.util.extension.map
...
interface ITranslateSrc {
    ...
    infix fun TranslateType.key(string: String): TranslationKey {
        return TranslationKey.getOrPut(this, transform(string), string)
    }

    infix fun TranslateType.key(pair: Pair<String, String>): TranslationKey {
        return TranslationKey.getOrPut(this, transform(pair.first), pair.second)
    }
}
