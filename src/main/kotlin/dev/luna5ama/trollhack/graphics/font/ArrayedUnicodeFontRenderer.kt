...
import dev.luna5ama.trollhack.utils.extension.profiler
...
import dev.luna5ama.trollhack.graphics.texture.*
...
class ArrayedUnicodeFontRenderer private constructor(
    ...
    init {
        if (!RS.compatibility.arbSparseTexture)
            TrollHackMod.LOGGER.warn("Using Arrayed UnicodeFontRenderer " +
    ...
    private fun initChunk(chunk: Int): Boolean {
        ...
        if (!::atlas.isInitialized) {
            ...
            TrollHackMod.LOGGER.debug("Allocate ${65536 / chunkSize} layers, $imgSize, $imgSize")
        }
        ...
        if (RS.compatibility.arbSparseTexture) {
            TrollHackMod.LOGGER.debug("Commit page 0, 0, 0, $chunk, $imgSize, $imgSize, 1, true")
