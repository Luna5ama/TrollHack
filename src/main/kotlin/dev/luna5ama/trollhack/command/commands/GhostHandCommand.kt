...
import dev.luna5ama.trollhack.module.modules.player.GhostHand
                ...
                execute("Add a block to block list") {
                    ...
                    if (GhostHand.blockList.contains(blockName)) {
                        NoSpamMessage.sendError(
                            GhostHandCommand,
                            "${formatValue(blockName)} is already added to the visible block list"
                        )
                    } else {
                        GhostHand.blockList.editValue { it.add(blockName) }
                        NoSpamMessage.sendMessage(
                            GhostHandCommand,
                            "${formatValue(blockName)} has been added to the visible block list"
                        )
                    }
                }
            }
            ...
            block("block") { blockArg ->
                execute("Remove a block from block list") {
                    ...
                    if (!GhostHand.blockList.contains(blockName)) {
                        NoSpamMessage.sendError(
                            GhostHandCommand,
                            "You do not have ${formatValue(blockName)} added to xray visible block list"
                        )
                    } else {
                        GhostHand.blockList.editValue { it.remove(blockName) }
                        NoSpamMessage.sendMessage(
                            GhostHandCommand,
                            "Removed ${formatValue(blockName)} from xray visible block list"
                        )
                    }
                }
            }
